...
The core styles and themes are located in `@asafmalin/ngrid/themes` which exposes a basic CSS theme mandatory for **nGrid** to render properly, it comes in 2 flavours:
...
```scss
@import '@asafmalin/ngrid/themes/default-light.css';
```
...
```html
<link href="node_modules/@asafmalin/ngrid/themes/default-light.css" rel="stylesheet">
```

I> Most chances are you're using a custom UI plugin (e.g `@asafmalin/ngrid-material`).  
...
> Note that pre-built CSS themes are located in `@asafmalin/ngrid/themes` and the SCSS theming utilities are located in `@asafmalin/ngrid`
...
- [@asafmalin/ngrid-material](../../../plugins/ngrid-material/theming) - 4 *Material Design* themes and material design SCSS theming framework
- [@asafmalin/ngrid-bootstrap](../../../plugins/ngrid-bootstrap/theming) - Bootstrap themes and bootstrap SCSS theming framework
